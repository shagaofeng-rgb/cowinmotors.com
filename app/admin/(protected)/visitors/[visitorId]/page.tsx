...
export default async function AdminVisitorDetailPage({ params, searchParams }: { params: Promise<{ visitorId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [{ visitorId }, rawParams] = await Promise.all([params, searchParams]);
  const range = getAdminDateRange(rawParams);
  const detail = await getVisitorProfile(visitorId, range);
  if (!detail) notFound();
  const { profile, events } = detail;
  return <div className="admin-page admin-detail-page">
    <header className="admin-page-head"><div><Link className="admin-back-link" href="/admin/visitors">返回访客记录</Link><p className="eyebrow">匿名访客详情</p><h1>{profile.displayId}</h1><p>{profile.country || "Unknown"}{profile.region ? ` · ${profile.region}` : ""} · {profile.maskedIp} · 最后活动 {new Date(profile.lastSeen).toLocaleString("zh-CN", { hour12: false })}</p></div><div className="admin-page-actions"><AdminDateRangeFilter range={range} /><div className="admin-badge-list">{profile.labels.map((label) => <span className="admin-status good" key={label}>{label}</span>)}</div></div></header>
