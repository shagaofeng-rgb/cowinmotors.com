export const UI_ASSETS = {
  logo: "/assets/ui/shared/brand/cowinmotors-logo-header.png",
  headlightHero: "/assets/ui/product-images/headlights/hero/headlights-hero-product_white-bg.png",
  tailLightHero: "/assets/ui/product-images/tail-lights/hero/tail-lights-hero-product_white-bg.png",
  exhaustHero: "/assets/ui/product-images/exhaust-systems/hero/exhaust-systems-hero-product_white-bg.png",
  wheelHero: "/assets/ui/product-images/wheels/hero/wheels-hero-product_white-bg.png",
  bodyKitHero: "/assets/ui/product-images/body-kits/hero/body-kits-hero-product_white-bg.png",
  moreParts: "/assets/ui/illustrations/automotive/headlight-wireframe-car.png",
  supportNetwork: "/assets/ui/illustrations/support/global-support-car-network.png",
  supportHeadset: "/assets/ui/illustrations/support/support-headset.png",
  newsLighting: "/assets/ui/photography/news/article-lighting-trends.png",
  service: {
    quality: "/assets/ui/photography/service-cards/quality-inspection.png",
    packaging: "/assets/ui/photography/service-cards/secure-packaging.png",
    sourcing: "/assets/ui/photography/service-cards/sourcing-coordination.png",
    shipping: "/assets/ui/photography/service-cards/global-shipping.png",
    afterSales: "/assets/ui/photography/service-cards/after-sales-support.png",
  },
  bodyKits: [
    "/assets/ui/product-images/body-kits/catalog/body-kit-01_white-bg.png",
    "/assets/ui/product-images/body-kits/catalog/body-kit-02_white-bg.png",
    "/assets/ui/product-images/body-kits/catalog/body-kit-03_white-bg.png",
    "/assets/ui/product-images/body-kits/catalog/body-kit-04_white-bg.png",
    "/assets/ui/product-images/body-kits/catalog/body-kit-05_white-bg.png",
  ],
};
